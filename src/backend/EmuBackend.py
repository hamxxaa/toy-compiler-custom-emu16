...
class EmuBackend:
    ...
    #   0x0008-0x7FFF  Data section (globals + arrays incl. the ROM-shipped font, streamed sprite
    #                  sheets, AND the runtime map buffer for pak-loaded maps — 32 KB, bumped from
    #                  24 KB when the legacy VRAM/PRAM path was reclaimed; VRAM/PRAM no longer
    #                  exist, every ROM drives the PPU now)
    #   0x8000-0xFFFB  Code section (functions, grows up from 0x8000; stack shares this region,
    #                  growing DOWN from the top)
    #   0xFFFC         Stack base (grows DOWN from here; floor = STACK_FLOOR in definitions.h)
    #   0xFFFE         SYSCALL_PORT (write-triggered host call)
    #   0xFFFF         INPUT (hardware mapped)
    ...
    CODE_START_ADDRESS = 0x8000
    DATA_START_ADDRESS = 0x0008
    DATA_END_ADDRESS   = 0x8000   # data grows up to code start (32 KB for globals + streamed sheets)
    STACK_START_ADDRESS = 0xFFFC
    SYSCALL_PORT = 0xFFFE
    INPUT_ADDRESS = 0xFFFF
